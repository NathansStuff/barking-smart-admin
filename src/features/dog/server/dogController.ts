...
export async function createDogHandler(req: NextRequest): Promise<NextResponse> {
...
export async function getDogHandler(req: NextRequest): Promise<NextResponse> {
  ...
  if (!dog) {
    return NextResponse.json({ message: 'Dog not found' }, { status: ResponseCode.NOT_FOUND });
  }
  return NextResponse.json(dog, { status: ResponseCode.OK });
}

// Handler to update a dog by ID
export async function updateDogHandler(req: NextRequest): Promise<NextResponse> {
  const id = getLastSegment(req.nextUrl.pathname);
  const data = await req.json();
  const dateOfBirth = new Date(data.dateOfBirth);
...
export async function deleteDogHandler(req: NextRequest): Promise<NextResponse> {
  const id = getLastSegment(req.nextUrl.pathname);
  await deleteDogByIdService(id);
  return NextResponse.json({ message: 'Dog deleted successfully' }, { status: ResponseCode.OK });
}
